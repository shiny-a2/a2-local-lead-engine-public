"""Demonstrate the email chain using the REAL OpenAI client (Phase 6 -> 7 -> 8 -> 9).

Requires OPENAI_API_KEY in the environment / .env. Makes one real GPT call, runs the
rule judge, and queues for human review. Nothing is sent.
...
def main() -> None:
    settings = Settings(
        database_url="sqlite:///./a2_gptdemo.db",
    ...
    if not settings.openai_api_key:
        print("No OPENAI_API_KEY found in .env/env.")
        return
    ...
    gen = EmailWriterService(session, settings).generate(campaign.slug, None, 10, commit=True)
